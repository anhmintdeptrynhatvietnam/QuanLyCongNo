...
async function main() {
  try {
    ...
    // A session bound by session-init keeps its state in the identity-keyed
    // store; anything older still reads and writes flat files under the raw id.
    const sessionId = data.session_id;
    const sessionContext = sessionId
      ? createSessionStateContext({
          sessionId,
          cwd: process.env.FIS_PROJECT_ROOT || data.workspace?.current_dir || data.cwd || process.cwd(),
          requireBinding: true
        })
      : null;
    const stateTarget = sessionContext || sessionId || null;
    ...
    try {
      if (stateTarget) {
        const session = readSessionState(stateTarget);
        const planPath = session?.activePlan?.trim();
        if (planPath) activePlan = extractActivePlanLabel(planPath);
        transcript = readActivitySnapshot(stateTarget, readSessionState) || transcript;
      }
    } catch {}
    ...
    if (sessionId && contextSize > 0) {
      ...
      try {
        if (!sessionContext || !writeContextState(sessionContext, contextState)) {
          const contextDataPath = path.join(os.tmpdir(), `fis-context-${sessionId}.json`);
          fs.writeFileSync(contextDataPath, JSON.stringify(contextState));
        }
      } catch {}
    }
