...
- Pre-landing review (from the installed ship skill pipeline)
...
Always load: `checklists/base.md` and `checklists/ai-generated.md`
(diffs produced through this kit are AI-assisted; `ai-generated.md` also covers the FIS QMS 04.05 checklist items)
...
For each critical issue, use `ask_user capability` (the `AskUserQuestion` tool on this runtime):
...
## Integration with the installed ship skill

The ship pipeline calls this workflow at Step 4. Critical findings block the pipeline. Informational findings are included in the merge/pull request body.
