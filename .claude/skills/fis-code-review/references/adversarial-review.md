...
| **Defer** | Valid but low-risk, tracked for later | Open a tracking issue on the repository's provider (`glab` on GitLab, `gh` on GitHub) |
...
- Deferred: C (tracked via provider issues)
...
#### [N] SEVERITY -- CATEGORY -- file:line
**Attack:** How to trigger
**Verdict:** Defer -- [reason] → tracking issue #X
```

### 4. Fix Accepted Findings

- Critical: Block merge. Fix immediately via `/fis:fix` or manual edit.
