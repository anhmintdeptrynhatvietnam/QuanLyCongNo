# `/fis:handoff` Artifact Schema

The handoff artifact is a Markdown document with a fixed section order. This
schema is validated by `/fis:handover` before dispatch (by heading name,
...
`handoff-version` lets `/fis:handover` reject artifacts written by a future
incompatible schema.
...
Additional H2 sections may appear between required ones. `/fis:handover`
...
- **Source pointers** — bulleted list of paths and URLs the successor agent
  needs. Do not fabricate URLs. Repo-relative paths preferred. Merge request
  and pull request links are recorded in whatever form the remote's provider
  uses (GitLab or GitHub); never rewrite one provider's URL into the other's.
...
## Validation summary (used by `/fis:handover`)
...
Any failure is a hard blocker — `/fis:handover` refuses to dispatch.
