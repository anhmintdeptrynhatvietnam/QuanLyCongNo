# `/fis:handoff` Redaction Patterns
...
| GitLab PAT / job or runner token | `glpat-[A-Za-z0-9_-]{20,}`, `glrt-[A-Za-z0-9_-]{20,}`, `gldt-[A-Za-z0-9_-]{20,}` | `[REDACTED:gitlab-token]` |
...
gather that evidence itself before acting, and `/fis:handover` treats an
empty required section as a validation failure.

## Verification recipe (for the advisory reviewer)
