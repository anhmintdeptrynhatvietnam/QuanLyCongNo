...
user-invocable: true
when_to_use: "Use when a successor agent must resume this exact task and needs the mission, decisions, verification state, and blockers captured first."
category: utilities
...
metadata:
  author: fis-ai-kit
  version: "1.0.0"
  upstream: "Pinned MIT source archive: handoff@ce70edaa26247b84c2b9491a0cdb4964f65cf3a5 (rewritten for the FIS AI Kit continuation contract)"
---
...
composition belongs to [`/fis:handover`](../fis-handover/SKILL.md).

## Boundary vs `/fis:watzup`

- `/fis:handoff` = continuation contract for a **successor agent**. Session
  reasoning, decisions, verification state, exact next actions.
- `/fis:watzup` = status report for a **human**, derived from branches,
...
```bash
/fis:handoff
/fis:handoff "continue the OAuth callback fix"
/fis:handoff --output plans/handoffs/oauth-callback.md
/fis:handoff --include-diff --include-status
/fis:handoff --force --output plans/handoffs/oauth-callback.md
```
...
- If the project has no `plans/` root at all, ask the user for a safe output
  location through `ask_user capability` (the `AskUserQuestion` tool on this
  runtime) before writing. When the repository defines its own placement
  convention, invoke `/fis:project-organization` to resolve it.
...
- Also scan legacy `plans/reports/handoff-*.md` so artifacts written by earlier
  kit versions and by other tools keep being found.

## Required Artifact Structure

Every handoff document contains these nine H2 sections in this order — the
schema is validated by `/fis:handover` before dispatch. See
...
4. **Decisions and rationale** — decisions made, alternatives rejected, links
   to specs/issues/merge or pull requests/ADRs.
...
**Live workspace evidence** — probe read-only through `run_shell capability`
(the `Bash` tool on this runtime), one command at a time, before drafting
Current state:
...
**Git provider** — when an issue, merge request, or pull request needs a
lookup before it can be cited, resolve the provider from the remote first
(`git remote get-url origin`): GitLab, including self-hosted, uses `glab`
(for example `glab mr view <id>`); GitHub uses `gh` (for example
`gh pr view <id>`). Never assume one provider. If the matching CLI is missing
or unauthenticated, record the plain URL or ID instead and do not fabricate
the reference.
...
**When** `/fis:handoff --include-diff --include-status` runs.
...
**When** `/fis:handoff --output plans/handoffs/oauth-callback.md` runs.
...
- No runtime dispatch, prompt assembly for another agent, or preflight — use
  [`/fis:handover`](../fis-handover/SKILL.md).
- No human-facing project status derived from branches or history — use
  `/fis:watzup`.
- No plan authoring or ADR minting — use `/fis:plan`, `/fis:architecture`, or
  `/fis:docs`.
