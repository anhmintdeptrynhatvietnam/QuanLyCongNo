...
If invoked with context (test scope), proceed with testing. If invoked WITHOUT arguments, use `ask_user capability` (the `AskUserQuestion` tool on this runtime) to present available test operations:
...
Browser-based visual testing via `fis:agent-browser`, `fis:chrome-profile`, `fis:web-testing`, `fis:chrome-devtools`, or project-native Playwright/Vitest/k6 commands. Covers screenshots, responsive checks, accessibility audits, form automation, and console error collection.
...
6. For frontend: run UI tests via `fis:agent-browser`, `fis:chrome-profile`, `fis:web-testing`, `fis:chrome-devtools`, or project-native browser tests
...
- **Browser**: `fis:agent-browser` for live browser interaction without real user cookies; `fis:chrome-profile` for the user's actual Chrome login state, opened with `chrome-profile open --json` and bound by the returned selector; `fis:web-testing` or project-native Playwright/Vitest/k6 for repeatable UI tests; `fis:chrome-devtools` for direct Chrome DevTools scripting (screenshots, ARIA, console, network)
...
## Report Output
**IMPORTANT:** Invoke the installed project-organization skill to organize the outputs.
