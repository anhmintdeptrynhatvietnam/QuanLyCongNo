...
Use `fis:agent-browser` for live browser interaction when a fresh/tool-managed browser is enough. Use `fis:chrome-profile` only when the test needs the user's real Chrome profile, cookies, or already-logged-in state. Use `fis:web-testing` or project-native Playwright/Vitest/k6 commands for repeatable test runs. Use `fis:chrome-devtools` when the test needs direct Chrome DevTools scripting.
...
Prefer project-native auth helpers for repeatable tests. For ad-hoc browser driving with real user auth/cookies, invoke `fis:chrome-profile` and run:
...
### Alternative: Chrome DevTools scripting (`fis:chrome-devtools`)
When neither `agent-browser` nor a real Chrome profile fits, inject auth directly and drive the page with the skill's scripts:
