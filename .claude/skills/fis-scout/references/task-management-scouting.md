...
Track parallel scout agent execution on the live task-management surface —
`TaskCreate`, `TaskUpdate`, `TaskList` on this runtime.

Discover that surface at runtime; do not infer support from a cached tool list.
If it is unavailable, use the runtime's lightweight todo capability (`TodoWrite`)
or update the active plan instead. The active plan is the durable source of
truth; task tracking adds observability, not functionality.
