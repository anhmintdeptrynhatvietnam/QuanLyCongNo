...
when the active runtime permits the `delegate_agent` capability — the `Task`
tool on this runtime.
...
`read_file`, and `run_shell` — the `Grep`/`Glob`, `Read`, and `Bash` tools on
this runtime.
...
Use the native delegation capability:
