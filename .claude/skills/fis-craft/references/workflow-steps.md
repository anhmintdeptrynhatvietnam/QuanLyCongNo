...
it when available (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`, or `TodoWrite`
on a runtime without them). Otherwise, update the active plan directly. Plan
files are the durable source of truth, and every workflow step must work without
runtime task tracking.

**Delegation and questions are capabilities.** `delegate_agent capability(...)`
is the `Task` tool on this runtime and `ask_user capability` is
`AskUserQuestion`. Resolve them against the live tool surface rather than
...
**Fast:**
- Use the planning capability in fast mode (`/fis:plan --fast`) with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use the planning capability in parallel mode (`/fis:plan --parallel`) for dependency graph + file ownership matrix
...
- Use `ask_user capability` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run the planning capability's validate mode (`/fis:plan validate`)
...
- Use the multimodal capability (`/fis:ai-multimodal`) for image assets
...
1. **MUST** activate the installed project-management skill (MANDATORY) — run full sync-back for [plan-path]: reconcile completed runtime work with all phase files, backfill stale completed checkboxes across every phase, then update plan.md frontmatter/table progress. Do NOT only mark current phase.
...
Change plan state through the plan accessor whose path the session context
injects — `node <accessor> update <phase> --status <s>` and
`node <accessor> check <phase> <item>` — so `plan.md` and the phase files move
together. Read the script header for exact flags; it owns the syntax, not this
workflow. See `plan-state-files-first.md`.

**Fallback:** If the accessor is unavailable, edit plan.md directly —
...
- **MANDATORY DELEGATION:** Steps 4, 5, 6 MUST delegate via delegate_agent capability / skill activation. DO NOT implement directly.
  ...
  - Step 6: the installed project-management skill, conditional `docs-manager`, `git-manager`
