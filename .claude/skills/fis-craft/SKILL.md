...
— on this runtime that capability is the `Task` tool. Give it enough context to
answer in one reply; it does not interview.

**When the workflow reaches a merge/pull request** (e.g. handed off to the
installed ship skill): pass `--advice` to the downstream skill so supervision
persists across the handoff. Watch and fix CI until every required check is
green, then spawn `kongming` to review the whole implementation and post its
assessment plus concrete next steps as a comment directly on the request and the
...
- Route concrete bugs to `/fis:fix`; it frames intent first, then proves the root
...
If review/testing reveals a side effect, regression, or broken workflow, STOP. Use `ask_user capability` (the `AskUserQuestion` tool on this runtime) to present:
...
use it when available (`TaskCreate`/`TaskUpdate`/`TodoWrite` on this runtime).
Otherwise, update the active plan directly. Plan files are the durable source of
truth; do not infer support from cached tool lists.

**Plan resolution (files-first):** when the input is a plan path or an existing
plan is in scope, resolve it through the plan accessor whose path the session
context injects (`node <accessor> …`). Use the current-plan pointer (`use`)
first, falling back to `resolve` for the current repo/branch/worktree. Read
phase content with `show` (or the files directly) and mutate status only through
the accessor's file-mutating subcommands (`check`/`uncheck`/`update`) — never by
hand-editing the phases table, and never require a linked issue to resolve or
...
- **Finalize (MANDATORY - never skip):**
  1. **Activate the installed project-management skill (MANDATORY)** → run full plan sync-back across ALL `phase-XX-*.md` (not only current phase), update `plan.md` status/progress, refresh runtime tracking when available, generate progress report
  ...
  5. Run `/fis:journal` to write a concise technical journal entry upon completion — unless the shared "Journal step — opt-out" below applies.
...
Explicit `/fis:journal` invocation is unaffected. The rest of the Finalize block above stays MANDATORY.
...
| Finalize | the installed project-management skill; conditional `docs-manager`; configured git workflow | Project sync and docs-impact decision are mandatory |
...
- Pattern: `delegate_agent capability(subagent_type="[type]", prompt="[task]", description="[brief]")` — the `Task` tool on this runtime
...
- `references/plan-state-files-first.md` - Canonical plan-file model and accessor commands

## Workflow Position

**`/fis:outcome` classifies and chains; this skill implements.** When the router
routes an implementation link here, own the work inside that link and hand
control back rather than re-routing the rest of the chain yourself.

**Typically follows:** the installed planning skill (execute a plan), the installed brainstorm skill (implement agreed solution)
**Typically precedes:** the installed code-review skill (review after implementation), the installed test skill (validate changes)
**Related:** `/fis:fix` (alternative for bug fixes), the installed planning skill (create plan before crafting)
