...
Concrete taxonomy of "AI slop" code patterns the reviewer should flag, with
detection heuristics, fix guidance, and calibration on when **not** to flag.
Applies identically to GitLab MRs and GitHub PRs ("MR" below covers both).

## Why this matters

LLM-assisted contributions often produce code that *compiles, passes tests,
and looks reasonable per-file* — but pollutes the codebase at the aggregate.
Common drivers: pattern-matching without understanding constraints, authors
who don't read their own diffs, and agents that prioritize "doing something"
over "doing nothing when the right answer is no change". The result looks
harmless per instance but compounds. This reference gives the reviewer the
lens to spot it.
...
**Pattern**: MR adds a NOT NULL column, renames a field, changes a response shape — no migration, no backward-compat shim.
...
### 1.6 God-file growth
**Pattern**: A 180-line file grows to 450 in one MR. Project's stated size limit (often 200 lines) ignored.
...
## Section 3: Process slop (MR-level signals)

Look across the whole diff, not per-file.

### 3.1 Scope mismatch
**Signal**: MR title says "fix typo" but diff is +800/−60 across 12 files.
**What it means**: Author wasn't watching what the agent did, OR the title is wrong.
**Action**: Ask for the MR to be split into focused commits, or ask the author to rewrite the title/description.

### 3.2 Unrelated files
**Signal**: "Fix auth bug" MR also rewrites a logging helper, reorders imports in 5 unrelated files, bumps a dep.
**Action**: Ask the author to separate the unrelated changes into their own MR.
...
**Action**: Block until tests cover the new path, or the skips are justified in the MR description.
...
Slop findings are *judgment calls*. Bug findings are not. The reviewer LLM
tends to swing between "aggressive style cop" and "too polite to flag
anything". Both fail.
...
- **Abstractions can be intentional** if the author knows the second implementation is coming in the next MR. Ask, don't assume.
...
Rule of thumb: if you cannot articulate the **concrete cost** of the pattern
in this specific codebase, do not flag it.
