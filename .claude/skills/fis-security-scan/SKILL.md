...
| FIS Quality Gate | Report check + reminder | Fast | `references/fis-quality-gate.md` |
...
## FIS Quality Gate (FPT IS projects)

For projects under FIS QMS (08-QD), golive requires passing the corporate Quality Gate measured by PQA/CSC tools (SonarQube, Coverity, Black Duck, OWASP ZAP). Load `references/fis-quality-gate.md` when the user mentions golive, release readiness, or provides scan tool reports. The internal scan here never replaces those tools — it complements them and helps triage/fix findings.

## Scope Declaration

This skill handles: Secret detection, dependency auditing, common vulnerability patterns, FIS Quality Gate report checking (when reports are provided).
This skill does NOT handle: Penetration testing, runtime security analysis, infrastructure security, compliance audits, running SonarQube/Coverity/Black Duck/ZAP themselves.
