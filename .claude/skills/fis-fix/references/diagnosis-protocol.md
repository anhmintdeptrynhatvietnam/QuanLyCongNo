...
Test each hypothesis against evidence. When delegation is explicitly permitted,
spawn parallel `Explore` subagents through `delegate_agent` (the `Task` tool on
this runtime); otherwise check them in the main agent with search and read:

```
// Launch in SINGLE turn — max 3 parallel agents
delegate_agent capability("Explore", "Test hypothesis A: [specific search/check]", "Verify H-A")
delegate_agent capability("Explore", "Test hypothesis B: [specific search/check]", "Verify H-B")
delegate_agent capability("Explore", "Test hypothesis C: [specific search/check]", "Verify H-C")
```
