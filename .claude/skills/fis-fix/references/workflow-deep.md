...
**Mandatory:** Activate `fis:scout` skill or, when delegation is permitted,
launch 2-3 `Explore` subagents in parallel through `delegate_agent` (the `Task`
tool on this runtime). Discover the delegation tool on the live tool surface
before assuming a name:
```
delegate_agent capability("Explore", "Find error origin and affected components", "Trace error")
delegate_agent capability("Explore", "Find module boundaries and dependencies", "Map deps")
delegate_agent capability("Explore", "Find related tests and similar patterns", "Find patterns")
```
