...
Run typecheck and lint through the `run_shell` capability (the `Bash` tool on
this runtime). Delegate them to parallel workers only when the user explicitly
requested parallel delegation:
```
