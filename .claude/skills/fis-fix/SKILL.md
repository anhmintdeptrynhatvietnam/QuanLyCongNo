...
**When the workflow reaches a merge/pull request** (e.g. a CI-failure fix
shipped for review): when handing off to a downstream skill, pass `--advice`
along so supervision persists. Watch and fix CI until every required check is
...
If ANY item is vague ("probably", "I think", "something with…"), use `ask_user capability` (the `AskUserQuestion` tool on this runtime) to gather missing facts (logs, repro, env) OR run more scout/debug — never guess.
...
See `references/selection-workflow.md` for `ask_user capability` format.
...
HARD-GATE-EXACT-ROOT-CAUSE above is the authoritative diagnosis protocol; see
`references/diagnosis-protocol.md` for the supporting methodology.
...
**Progress orchestration (Moderate+ only):** After classifying, record all phases and their dependencies upfront. See `references/task-orchestration.md`.
...
- For multiple viable repairs or an architecture decision, activate
  `fis:brainstorm`, compare trade-offs, and resolve the direction before
  implementation.
...
6. **Parallel verification:** Launch `run_shell capability` agents (the `Bash` tool on this runtime) for typecheck + lint + build + test.
...
2. **Activate the installed project-management skill (MANDATORY)** → sync plan status (if the fix is part of a plan), update progress, refresh runtime tracking when available, generate status report
...
6. Run `/fis:journal` to write a concise technical journal entry upon completion — unless the shared "Journal step — opt-out" below applies.
...
- `fis:project-management` — MANDATORY for sync-back and progress tracking, every fix

**Conditional:**
- `fis:problem-solving` — auto-triggers when 2+ hypotheses fail in Step 2
- `fis:brainstorm` — after diagnosis when multiple valid approaches or an architecture decision remain
...
Load as needed:
- `references/selection-workflow.md` - `ask_user capability` format for mode
...
- `references/task-orchestration.md` - Progress-tracking patterns for moderate+ workflows
...
- `references/parallel-exploration.md` - Parallel Explore/shell/delegation coordination patterns
...
**Related:** `/fis:craft` (alternative for feature work), the installed debug skill (diagnose before fixing)
