...
those guidelines for response structure and explanation depth. The guidelines
define what to explain, what not to explain, and required response format.
...
- Ask a concise question only when a missing answer would materially change the
  result, safety boundary, or public contract and cannot be discovered. Ask
  through `ask_user capability` (the `AskUserQuestion` tool on this runtime).
...
Before deep-diving, assess whether the request covers multiple independent
subsystems:

- If the request describes 3+ independent concerns (e.g. "build a platform with
  chat, billing, analytics"), flag it immediately.
- Help the user decompose into sub-projects: identify pieces, relationships,
  build order.
- Each sub-project gets its own brainstorm → plan → implement cycle.
- Do not spend questions refining details of a project that needs decomposition
  first.
...
| Thought | Reality |
|---------|---------|
| "This is too simple to need a contract" | Simple projects = most wasted work from unexamined assumptions. |
| "I already know the solution" | Then writing down outcome, constraints, non-goals, and acceptance takes 30 seconds. Do it. |
| "The user wants action, not talk" | Bad action wastes more time than a bounded contract. |
| "Let me explore the code first" | The contract tells you WHAT to explore. Frame it, then inspect. |
| "I'll just prototype quickly" | Prototypes become production code. Agree the outcome first. |

## Supporting capabilities

Reach for these when the decision needs evidence the repository alone cannot
give. Resolve each against the live installed-skill catalog:

- `planner` agent — research industry practice and proven solutions
- `docs-manager` agent — understand existing project implementation and constraints
- Web search — find efficient approaches and others' experience
- `fis:docs-seeker` — read current documentation of external plugins/packages
- `fis:ai-multimodal` — analyze visual materials and mockups
- `psql` — inspect current database structure and existing data
- `fis:sequential-thinking` — structured analysis for complex problems
- `fis:scout` — discover relevant files and code patterns before claiming feasibility
...
- feature or documentation delivery: the installed plan skill, then `/fis:craft`;
- diagnosed bug: `/fis:fix`;
...
(the `## Naming` section in the injected context); do not create a report merely
to satisfy the gate. When you do write one, invoke `/fis:project-organization`
to place it, and include:
...
- **UI/UX mockups with annotations (required when the topic touches UI/UX):**
  ...
  spacing, and component states from the project design guidelines when present,
  otherwise a restrained built-in editorial contract. Add callouts tying each
  element to design tokens, interaction states, and the acceptance evidence it
  satisfies.
...
**When the workflow reaches a merge/pull request** (here, via the handed-off
plan/craft/fix workflow): pass `--advice` to the downstream skill so supervision
...
- Prioritize long-term maintainability over short-term convenience; weigh
  technical excellence against business pragmatism.
- List unresolved questions last when any remain.

## Workflow position

**Typically precedes:** the installed plan skill or `/fis:craft`.
