...
def validate_frontmatter(frontmatter: dict[str, Any], skill_path: str = "") -> list[str]:
    ...
    for flag in ("user-invocable", "disable-model-invocation"):
        value = frontmatter.get(flag)
        if value is not None and not isinstance(value, bool):
            errors.append(f"{prefix}'{flag}' must be a boolean")

    when_to_use = frontmatter.get("when_to_use")
    if when_to_use is not None:
        if not isinstance(when_to_use, str) or len(when_to_use) > 300:
            errors.append(f"{prefix}'when_to_use' must be a string <= 300 chars")
    ...
    _KNOWN_KEYS = frozenset({
        "name", "description", "argument-hint", "user-invocable", "when_to_use",
        "disable-model-invocation",
        "license", "languages", "allowed-tools", "category", "keywords",
        "requires", "related", "maturity", "metadata",
    })
