...
// Usage cache file path (written by usage-quota-cache-refresh.cjs hook)
 ...
 * @param {string} [options.baseDir] - Working directory to resolve from and against
 * @returns {Object} Plan context object
 */
function buildPlanContext(sessionId, config, options = {}) {
  const { plan, paths } = config;
  const cwd = options.baseDir || undefined;
  const gitBranch = getGitBranch(cwd);
  const resolved = resolvePlanPath(sessionId, config, cwd ? { cwd } : {});
  // baseDir goes into getReportsPath rather than being joined onto its result: a
  // directive plan path is already absolute, and joining concatenates.
  const reportsPath = getReportsPath(resolved.path, resolved.resolvedBy, plan, paths, options.baseDir || null);

  // Compute naming pattern directly for reliable injection
  const namePattern = resolveNamingPattern(plan, gitBranch);

  // Session state and the worktree pointer are both explicit acts, so both are
  // directive. A branch-name match is a guess and stays a suggestion.
  const planLine = isDirectivePlan(resolved.resolvedBy)
    ? `- Plan: ${resolved.path}`
 ...
 * @param {string|Object|null} sessionId - Session identifier or bound session context
 * @param {string|null} [scopeKey='session'] - Scope key for cwd/transcript-aware dedup
...
/**
 * Read usage limits from cache file (written by usage-quota-cache-refresh.cjs)
...
/** Reads the pre-binding statusline temp file. */
function readLegacyContextState(sessionId) {
 ...
 * @param {string|Object} sessionId - Session ID, or a bound session context
...
function buildContextSection(sessionId) {
  ...
  try {
    // A bound session keeps this in the identity-keyed store; an unbound one
    // still has the flat temp file the statusline falls back to.
    const data = typeof sessionId === 'string'
      ? readLegacyContextState(sessionId)
      : readContextState(sessionId);
    if (!data) return [];
...
/**
 * Path to the plan accessor, or null when it is not installed.
 *
 * Injected rather than written into skill prose because DAI installs the payload
 * at project or global scope, so no fixed relative path is correct everywhere.
 *
 * Two forms, for two consumers. The short `.claude/scripts/…` form reads well in
 * injected prose. Anything that will be executed needs the absolute form: `~`
 * is not expanded by node, and a relative path breaks the moment the agent works
 * in a subdirectory — the case this indirection exists for.
 *
 * @param {Object} [options]
 * @param {boolean} [options.absolute=false] - Return a path safe to execute
 * @param {string} [options.configDirName='.claude'] - Config directory name
 * @returns {string|null} Accessor path
 */
function resolvePlanAccessor(options = {}) {
  const { absolute = false, configDirName = '.claude' } = options;
  const own = path.resolve(__dirname, '..', '..', 'scripts', 'fis-plan.cjs');

  if (absolute) return fs.existsSync(own) ? own : null;

  const conventional = resolveScriptPath('fis-plan.cjs', configDirName);
 ...
 * @param {string|null} [params.planAccessor] - Path to the plan accessor script
 * @returns {string[]} Lines for plan context section
 */
function buildPlanContextSection({ planLine, reportsPath, gitBranch, validationMode, validationMin, validationMax, planAccessor }) {
  const lines = [
    `## Plan Context`,
    planLine,
    `- Reports: ${reportsPath}`
  ...
  // Absolute, because this line is presented as a command to run. The short
  // `.claude/scripts/…` form only resolves from the project root, and the agent
  // may well be working below it.
  const accessor = planAccessor === undefined ? resolvePlanAccessor({ absolute: true }) : planAccessor;
  if (accessor) {
    lines.push(`- Plan accessor: \`node ${accessor} <use|resolve|status|show|check|update>\``);
    lines.push(`- Change plan status through the accessor so plan.md and the phase files stay in sync`);
  }
...
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.claude', baseDir } = {}) {
  // Load config if not provided, from the caller's directory so a monorepo
  // package's own settings apply
  const cfg = config || loadConfig({ includeProject: false, includeAssertions: false, cwd: baseDir || process.cwd() });
  ...
  // Build plan context. baseDir is passed in so the reports path comes back already
  // absolute; joining it here would double a plan path that is absolute already.
  const planCtx = buildPlanContext(sessionId, cfg, { baseDir: effectiveBaseDir });
