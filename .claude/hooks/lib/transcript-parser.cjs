 ...
 *
 * Callers running inside a hook can bound the work: a transcript grows without
 * limit over a long session, and reading all of it can outlast the event. With
 * no options the whole file is read.
 *
 * Note that starting at a byte offset usually lands mid-line, so the first
 * fragment fails to parse and counts as an invalid line.
 *
 * @param {string} transcriptPath - Path to transcript file
 * @param {Object} [options]
 * @param {number} [options.start] - Byte offset to start reading at
 * @param {number} [options.end] - Inclusive byte offset to stop reading at
 * @param {number} [options.maxLines] - Stop after this many lines
 * @param {number} [options.deadline] - Epoch ms after which parsing stops
