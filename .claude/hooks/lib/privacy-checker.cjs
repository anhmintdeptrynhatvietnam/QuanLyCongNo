...
/**
 * Load .fisrc.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .fisrc.json (defaults to .claude in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.fisrc.json')
      : path.join(process.cwd(), '.claude', '.fisrc.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .fisrc.json config
