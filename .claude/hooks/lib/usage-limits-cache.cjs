...
const DEFAULT_USER_AGENT = 'fis-ai-kit-engineer/1.0';
const DEFAULT_ELIGIBILITY_CACHE_TTL_MS = 60_000;

function getUsageCachePath() {
  return process.env.FIS_USAGE_CACHE_PATH || path.join(os.tmpdir(), 'fis-usage-limits-cache.json');
}

function getQuotaEligibilityCachePath() {
  return process.env.FIS_USAGE_ELIGIBILITY_CACHE_PATH || `${getUsageCachePath()}.eligibility`;
}
