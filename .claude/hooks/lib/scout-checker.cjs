...
/**
 * Find an optional project-local .fisignore at the git root config directory.
 ...
 * @returns {string|null}
 */
function findProjectFisignore(startDir, configDirName) {
  if (!configDirName || typeof configDirName !== 'string') return null;
  const gitRoot = findGitRoot(startDir);
  if (!gitRoot) return null;
  const candidate = path.join(gitRoot, configDirName, '.fisignore');
 ...
 * @param {string} [params.options.fisignorePath] - Path to .fisignore file
 * @param {string} [params.options.projectFisignorePath] - Explicit project-local .fisignore path
 * @param {string} [params.options.claudeDir] - Path to .claude or .opencode directory
 * @param {string} [params.options.cwd] - Working directory used to discover a project .fisignore
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  ...
  // Resolve .fisignore path
  const resolvedFisignorePath = fisignorePath || path.join(claudeDir, '.fisignore');
  const discoveredProjectFisignorePath = projectFisignorePath || findProjectFisignore(cwd, projectConfigDirName);
  const resolvedProjectFisignorePath = discoveredProjectFisignorePath
    && path.resolve(discoveredProjectFisignorePath) !== path.resolve(resolvedFisignorePath)
      ? discoveredProjectFisignorePath
      : null;
  const configPath = resolvedProjectFisignorePath || resolvedFisignorePath;

  // Load patterns and create matcher
  const patterns = loadPatterns(resolvedFisignorePath, resolvedProjectFisignorePath);
