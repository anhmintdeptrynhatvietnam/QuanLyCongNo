  ...
  try {
    ...
    if (toolName === 'Edit' || toolName === 'Write') {
      ...
      if (editingTableStatus) {
        ...
        // The accessor writes plan.md and the phase file together under a lock, so
        // point at it rather than describing how to hand-edit the cell safely.
        const accessor = resolvePlanAccessor({ absolute: true });
        warnings.push(accessor
          ? `Use the plan accessor instead of editing status cells directly: node ${accessor} update <phase> --status <status>`
          : 'Use the plan accessor (scripts/fis-plan.cjs update <phase> --status <status>) instead of editing status cells directly.');
      }
    }
