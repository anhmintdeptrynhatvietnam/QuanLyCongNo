...
try {
  ...
  const { resolveSkillsVenv, resolvePlanAccessor } = require('./lib/context-builder.cjs');
...
async function main() {
  ...
  try {
    ...
    // From the subagent's directory, so a monorepo package's own settings apply.
    const config = loadConfig({ includeProject: false, includeAssertions: false, cwd: effectiveCwd });
    ...
    // Resolved from the subagent's own directory, like the git calls above: the
    // pointer and the branch match both belong to its worktree, not the parent's.
    const resolved = resolvePlanPath(sessionId, config, { cwd: effectiveCwd });
    const reportsPath = getReportsPath(resolved.path, resolved.resolvedBy, config.plan, config.paths, baseDir);
    const activePlan = isDirectivePlan(resolved.resolvedBy) ? resolved.path : '';
    ...
    if (activePlan) {
      ...
      // Injected because the payload may be installed at project or global scope,
      // so a subagent cannot assume where the accessor lives. Absolute, because a
      // subagent runs it from its own working directory.
      const planAccessor = resolvePlanAccessor({ absolute: true });
      if (planAccessor) {
        lines.push(`- Plan accessor: \`node ${planAccessor} <show|status|check|update>\` — the only way to change plan status`);
      }
