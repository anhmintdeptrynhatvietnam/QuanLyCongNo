...
/**
 * Build FIS stack context from environment variables
 * Set by session-init.cjs, available to subagents via SubagentStart
 */
function buildFisContext() {
  const ctx = [];
  const env = process.env;

  if (env.FIS_REPORTS_PATH) ctx.push(`Reports: ${env.FIS_REPORTS_PATH}`);
  if (env.FIS_PLANS_PATH) ctx.push(`Plans: ${env.FIS_PLANS_PATH}`);
  if (env.FIS_PROJECT_ROOT) ctx.push(`Project: ${env.FIS_PROJECT_ROOT}`);
  if (env.FIS_NAME_PATTERN) ctx.push(`Naming: ${env.FIS_NAME_PATTERN}`);
  if (env.FIS_GIT_BRANCH) ctx.push(`Branch: ${env.FIS_GIT_BRANCH}`);
  if (env.FIS_ACTIVE_PLAN) ctx.push(`Active plan: ${env.FIS_ACTIVE_PLAN}`);
...
function main() {
  try {
    ...
    // FIS stack context
    const fisCtx = buildFisContext();
    if (fisCtx.length > 0) {
      lines.push('');
      lines.push('## FIS Context');
      lines.push(...fisCtx);
    }
