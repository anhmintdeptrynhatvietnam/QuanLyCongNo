 ...
 *
 * Blocks access to directories listed in the shipped .claude/.fisignore baseline
 * plus an optional project-local .claude/.fisignore override at the git root.
 ...
 *
 * Configuration:
 * - Edit .claude/.fisignore to customize shipped baseline patterns
 * - Add <project-root>/.claude/.fisignore to override locally without changing the baseline
  ...
  try {
    ...
    const result = checkScoutBlock({
      ...
      options: {
        ...
        fisignorePath: path.join(claudeDir, '.fisignore'),
