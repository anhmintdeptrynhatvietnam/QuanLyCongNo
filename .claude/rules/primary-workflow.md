...
- For ambiguous workflow sequence, follow `skill-workflow-routing.md` and resolve
  each capability against the runtime's live skill catalog.
...
- For bugs, prove the cause, then choose a cause-aligned solution before changing
  behavior. Invoke the full brainstorm capability when multiple viable fixes or an
