...
When the current session or a subagent runs on a model below the strongest one
available and hits a hard problem — repeated failed attempts, a high-stakes
design fork, or fuzzy requirements — spawn the `kongming` agent for counsel
instead of switching the session model. `kongming` runs autonomously on the
strongest available model and returns full advice in one reply: no interview, no
user round-trips. Give it the task, the evidence gathered so far, the approaches
already tried, and the specific question. It advises only; the caller stays
responsible for the implementation.

For interview-driven advisory where the user participates in the exchange, use
the `advisor` agent instead.
